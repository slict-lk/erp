...
export default function EditProductPage() {
    ...
    useEffect(() => {
        ...
        if (params.id) {
            setFetching(true);
            fetch(`/api/spareparts/products/${params.id}`)
                .then(res => {
                ...
                .then(data => {
                    ...
                    setImages(p.images || []);
                    if (p.aliases) {
                        setAliases(p.aliases.map((a: any) => ({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const res = await fetch(`/api/spareparts/products/${params.id}`, {
                method: 'PATCH',
                ...
                body: JSON.stringify({
                    ...
                    salePrice: parseFloat(String(formData.salePrice)) || 0,
                    costPrice: parseFloat(String(formData.costPrice)) || 0,
                    stockQty: parseFloat(String(formData.stockQty)) || 0,
                    minStockQty: parseFloat(String(formData.minStockQty)) || 0,
                    taxCategoryId: formData.taxCategoryId || null,
                    partNumber: formData.partNumber || null,
                    compatibleModels: compatibleModelsArray,
                    condition: formData.condition || 'NEW',
                    images: images,
                    isActive: true, // Keep absolute active on edit? Or should we allow deactivation? For now keep true as per POST
                    aliases: aliases.filter(a => a.aliasNumber.trim() !== '')
                }),
            });
